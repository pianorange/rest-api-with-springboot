...
※의존성 추가하는 순간 모든 요청에 대한 인증 필요해짐 (의존성 추가하기전에 만든 controller테스트 메소드 전부실패)

스프링 시큐리티 의존성에 들어있으면 스프링 부트는 스프링 시큐리티용 자동 설정해줌.
-> 모든 요청 인증 필요로 하게되고 , 스프링 시큐리티가 사용자를 하나 임의로 인메모리로 만들어 줌

#### EventController Class Authorization Setting
----
- 로그인 없이 접근 가능
  - GET/api/events
  - GET/api/events/{id}
...
- ResourceServer : 리소스 요청 인증 처리 (OAuth2 토큰검사)
